...
public class TournamentEngine {
    ...
    public TournamentResult runTournament() {
        ...
        while (hero.isAlive() && opponent.isAlive() && round < maxRounds) {
            ...
            if (hero.getHealth() <= 60 && hero.getHealPotions() > 0) {
                actionQueue.enqueue(new HealCommand(hero, 20));
            }

            actionQueue.enqueue(new DefendCommand(hero, 0.10));

            result.addLine("[Round " + round + "] Queued actions: " + actionQueue.getCommandDescriptions());

            actionQueue.executeAll();

            if (opponent.isAlive()) {
                result.addLine("[Round " + round + "] " + opponent.getName() + " attacks " + hero.getName() + "!");
